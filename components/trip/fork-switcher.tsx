...
import { GitBranch, Plus, Pencil, Trash2, BarChart2, Check, ChevronDown } from "lucide-react";
...
export function ForkSwitcher({ tripId, forks, phase }: ForkSwitcherProps) {
  ...
  function handleCreateFork() {
    startCreateTransition(async () => {
      const result = await createFork(tripId);
      if (result.success) {
        navigateToPlan(result.forkId);
      }
    });
  }
        ...
        <DropdownMenuContent align="end" className="w-56">
          ...
          {forks.map((fork) => (
            ...
            >
              ...
              {/* Rename + discard actions for this fork */}
              <span
          ...
          <DropdownMenuItem
            ...
            onSelect={atCap ? undefined : handleCreateFork}
          >
            <Plus className="size-4 shrink-0" aria-hidden="true" />
            {atCap ? (
              <span className="text-muted-foreground">
                {createPending ? "Creating…" : "Discard a variant first"}
              </span>
            ) : (
              <span>{createPending ? "Creating…" : "New variant"}</span>
            )}
          </DropdownMenuItem>
