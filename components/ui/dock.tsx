...
export interface DockItem {
  ...
  label: string;
  muted?: boolean;
  /** Overrides the default path.startsWith(href) active check — needed for exact-match rules and hrefs carrying a query string. */
...
function Dock({ items, children, className, "aria-label": ariaLabel = "Trip" }: { items: DockItem[]; children?: React.ReactNode; className?: string; "aria-label"?: string }) {
  ...
  return (
    <nav aria-label={ariaLabel} className={cn("island hidden w-24 shrink-0 flex-col items-center gap-2 border-r-2 border-border bg-sun py-5 md:flex", className)}>
