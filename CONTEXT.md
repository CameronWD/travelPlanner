...
**Digest**:
The once-a-day push TEEPEE sends a Traveller who has switched it on (per person, per Trip, from either the Trip's Settings or **Account**, which are two views of the one switch) to every **Device** they have enabled — the single outward interruption the app allows itself. It carries whatever is true that day and nothing more: a **Cost**'s **Due date** on each of the three days before it and on the day, **Checklist** items due or overdue, tomorrow's **Reminder**s, and — when tomorrow falls inside the Trip — tomorrow's **Transport**, check-in/check-out and timed **Item**s. It arrives in the evening, in the timezone reported by the **Device** that Traveller most recently used — one clock per person, not one per Device, so a Device left behind at home cannot drag the Digest to the wrong side of the day — at 8pm where the schedule reaches that zone cleanly, later within the evening where it does not, and is *not sent at all* when there is nothing to say. It is ordered by what is most costly to lose rather than by category, because it is capped in length and the cap takes the tail: tomorrow's plan first, then **Reminder**s (said once and never repeated), then **Due date** payments (repeated on each of the days before), then **Checklist** items (which reappear nightly until done, and so give way
