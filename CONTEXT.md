...
**Changeover day**:
A single calendar day two consecutive **Stop**s both claim — the day you leave one and arrive at the next, so it is at once the first Stop's **depart** date and the second's **arrive** date. It is *one* day on the **Timeline** and is planned as one: the **plan editor** renders it under both Stops' cards, and both cards show the same **Item**s whichever card they were added from. Every scheduled Item nonetheless has exactly one **owning Stop** — the Stop its **Cost** is counted against in the **Budget**'s per-Stop roll-up — marked plainly on the card that does not own it, so the day never hides where its money lands. An Item keeps its owning Stop for as long as that Stop's stay covers its date, and re-files only when moved onto a day the owner no longer covers. Distinct from a day that merely carries **Transport**: a changeover day is defined by two Stops sharing a date, not by the leg between them.
_Avoid_: Travel day (that is any day holding Transport or a check-out — the sense the **Digest**'s morning cover uses), transition day, overlap day, handover, split day
...
**Accommodation**:
A first-class place you sleep, attached to a Stop — check-in/out dates, address, confirmation number, cost. Usually one per Stop, occasionally more. Each of the check-in and check-out may optionally carry a **time**; a timed check-in/out sits in the day's Timeline at its time like any timed Item, while an untimed one falls back to a fixed reading order — check-out first thing on its day, check-in after that day's Transport. In the **plan editor** a Stop's Accommodation lives *inside* that Stop's card, under a section headed "Where you're staying" — a display heading only, not a term; the thing is still an Accommodation.
_Avoid_: Hotel, lodging, stay (note: "Stay" is too close to "Stop"); "staying" appears only in that section heading
...
**Category**:
The classifier on an Item used for colour-coding and budget grouping: Sightseeing, Food & Drink, Activity, Nightlife, Shopping, **Getting around**, **Place**, Other. **Getting around** covers movement that does *not* change your base — day-trip buses, local trains, metro passes — which is an Item, never a **Transport** (Transport is strictly Stop-to-Stop and drives the date engine; a day trip out and back is just timed Items on the day). **Place** is somewhere to *go* rather than something to *do* — a city, town, region or island ("Paris", "Tuscany") jotted on the **Wishlist** or the **Globe** before it has become a **Stop**; it is deliberately broader than "city". The same set classifies Globe **Markers**. In the **plan editor** a Stop's things to do are shown grouped by Category; on a day the **Anytime** bucket is grouped by Category while timed Items stay in time order.
_Avoid_: Type, tag, label; Transit/Transport as a category name (too close to the **Transport** entity); City (use **Place**)
...
**Settlement**:
*When in the journey* a **Cost** gets paid, as one of two kinds the Traveller sets on the Cost: **Before you go** (flights, hotels paid at booking, insurance — the default) or **On the trip** (a city tax at the desk, a hotel that charges at check-out, meals). It is a plain choice, never derived from dates: a **Due date** says *which day* money leaves, a Settlement says *which side of departure* it belongs to, and a Cost may have either, both or neither. The **Budget** rolls the two kinds up separately so "what we still owe before we leave" and "what we'll pay while we're away" are two honest numbers; **Upcoming payments** only ever concern the *Before you go* kind.
_Avoid_: Payment type, timing, phase (reserved for the Trip's **Phase**), pre-paid/post-paid
...
**Digest**:
The once-a-day push TEEPEE sends a Traveller who has switched it on (per person, per Trip, from either the Trip's Settings or **Account**, which are two views of the one switch) to every **Device** they have enabled — the single outward interruption the app allows itself. It carries whatever is true that day and nothing more: a **Cost**'s **Due date** on each of the three days before it and on the day, **Checklist** items due or overdue, tomorrow's **Reminder**s, and — when tomorrow falls inside the Trip — tomorrow's **Transport**, check-in/check-out and timed **Item**s. It arrives in the evening, in the timezone reported by the **Device** that Traveller most recently used — one clock per person, not one per Device, so a Device left behind at home cannot drag the Digest to the wrong side of the day — at 8pm where the schedule reaches that zone cleanly, later within the evening where it does not, and is *not sent at all* when there is nothing to say. It is ordered by what is most costly to lose rather than by category, because it is capped in length and the cap takes the tail: tomorrow's plan first, then **Reminder**s (said once and never repeated), then **Due date** payments (repeated on each of the days before), then **Checklist** items (which reappear nightly until done, and so give way first, and of which at most two ever appear however many are overdue). The count at the end says how much was left out, whether it fell off the end or was held back by that two-line limit. A second, shorter Digest goes out in the early morning on a travel day only — a day holding **Transport** or a check-out — as cover for an **Alarm** the Traveller's calendar app silently declined to fire; it carries that day's plan and nothing else, never repeating what last night's Digest already said. A Traveller can ask Settings for a one-off test Digest at any moment; unlike the scheduled ones it always arrives and always marks itself as a test, carrying that day's content where there is some and otherwise saying only that the push reached the device, because its job is proving the pipe works rather than reporting the day. Distinct from a **notification** (the bell's unread partner **Activity**, in-app only) and from an **Alarm** (fired by the Traveller's calendar app, not by TEEPEE).
_Avoid_: Notification, alert, summary, roundup, daily email

**Account**:
The one surface that belongs to a **Traveller** rather than to a Trip, reached from the avatar menu. It exists because some things are true of a *person* across every Trip they are on — their **Device**s above all — and rendering those inside one Trip's Settings makes an account-wide fact look like a Trip-wide one, which is how a Device that had quietly stopped working stayed invisible. It holds the Devices, and the list of which Trips send this Traveller a **Digest** — the same per-Trip switches the Trips' own Settings carry, gathered in one place so "am I getting digests, and where?" is answerable without visiting every Trip. Distinct from Trip **Settings**, which is where everything true of *one Trip* lives.
_Avoid_: Profile, preferences, user settings, my settings

**Device**:
One browser, on one machine, that a Traveller has granted permission to receive their **Digest** — the phone in their pocket, the laptop they plan from. Devices belong to the *person*, not to a Trip: one Device receives the Digests of every Trip that Traveller has switched on, and switching a Trip off silences it on all of them at once. A Traveller may have several, adds one by enabling the Digest *on that machine* (permission can only ever be granted from the device itself), and manages the set from **Account**. Two things about a Device can only be known by asking the Device: whether it still holds permission, and what timezone it is in — so it answers both on every visit, and the answer is what the Digest's send time is calculated from. A Device that stops answering has not necessarily gone: it may simply not have been opened. TEEPEE therefore reports a Device as *unseen since* a date and never quietly drops it, because a Device can be dead — permission revoked, the app deleted — while the push service it is registered with still accepts messages for it and reports success. Nothing in the delivery path can tell the difference; only the Device can.
_Avoid_: Subscription (that is the calendar-feed word), endpoint, push token, browser, session, installation

**Reminder**:
A dated note a Traveller writes for themselves on a Trip — "print the insurance docs" against 28 Nov. It carries a *date and never a time*: it is read out in the **Digest** the *evening before* that date, alongside tomorrow's plan, and shows on **Home** until the day itself. The evening-before delivery is the point — a note dated 28 Nov that only arrived at 9pm on the 28th would reach the Traveller as the day it was for was ending. It deliberately cannot be set to go off at a chosen moment, because TEEPEE dispatches only a few times a day and a promised 14:30 would be a lie — an **Alarm** is the thing that fires precisely. Shared between both **Traveller**s like the rest of the Trip. A Reminder may optionally be **about a Stop** ("sort the Denpasar visa" against 20 Nov): the Stop is a label that says what the note concerns and lets the Stop's card in the **plan editor** list it; it does not move the Reminder's date, which stays whatever the Traveller chose, usually well before the Stop. Distinct from a **Checklist** item (a tickable task with optional due date, which *persists until done* and keeps reappearing while overdue) — a Reminder is said once, the night before, and is not something you complete.
...
**Owner**:
A per-Trip membership role, held by whoever created the Trip. A Trip has exactly one Owner, and **the role cannot be transferred** — there is no hand-over feature, so the Owner can neither leave the Trip nor be removed from it, by themselves or by an **Admin**: a Trip carrying **Travellers** but no Owner could never again be deleted, duplicated, invited to, have a **Stop** deleted or a **Fork** promoted, with nothing to recover it. Distinct from an **Admin** (an `ADMIN_EMAILS` operator, recognised at request time, who is not a **Traveller** of the Trip at all until they join one — yet gains the same powers on any Trip they *are* a member of, see ADR 0045). Owner-only today: Delete, Duplicate (ADR 0045), creating an **Invite** (ADR 0052), removing a Traveller, deleting a **Stop** (including **Make it fit**'s Drop) and **Promote**-ing a **Fork** — every other Trip action is open to any Traveller on it. The **Globe** has its own, separate owner, and only they can create a Globe Invite; Globe ownership does not transfer either.
...
**Access request**:
A person without an account asking for one. It is about TEEPEE itself, not about any Trip: someone who is not yet a **Traveller** asks to be let in, and an **Admin** either approves them — after which they can sign in and plan their own Trips — or dismisses them. Sharply distinct from an **Invite**, which is Trip-scoped and created by that Trip's **Owner** to bring a named person onto that one Trip. An Invite says "join my Trip"; an Access request asks "may I use TEEPEE at all".
_Avoid_: invite, invitation (that's the Trip-scoped grant), signup, application, waitlist, approval (that's what an Admin does to a request, not the request itself)

**Share link**:
A labelled, read-only public window onto a Trip's **real plan**, made for one audience ("Mum & Dad", "The group chat") and handed out as a URL. A Trip may hold several at once, each with its own **scope** — which layers of detail that audience sees — and each revocable on its own, so cutting one audience off never touches another. A Share link grants no membership and no identity: whoever holds the URL sees the view, nothing more (contrast **Invite**, which becomes **Traveller** membership). Whatever the scope, some things are *never* shared on any link: money, **Note**s, confirmation numbers and booking references — and any **Item** a Traveller has marked **hidden from shares** (a per-Item switch, off by default, for the surprise dinner or the thing you'd rather the group chat didn't see; the Item stays fully visible to every Traveller and is only ever dropped from Share links, never from the plan). Always reads the real plan — never a **Fork**.
_Avoid_: bare "share" (say Share link), public link, guest access, viewer account (there is no viewer identity)
...
**Feedback note**:
A remark a **Traveller** writes *about TEEPEE* while using it — a defect, an annoyance, a "wouldn't it be nice". It is **not** trip content: it belongs to no **Trip** and no **Plan**, it never appears in any planning view, and nothing in the app reads it. It exists to be carried out of the app and worked on later. A Feedback note is free text plus the circumstances the app records for itself — the route it was written from, the Trip in view (if any), the author, the moment, the device, and the **site** it was written on — so it can be understood without the author being asked "where were you?". Each note carries a **status** (**Open**, **Done** or **Won't fix**); status changes when the work is actually done, not by the author tidying up. Sharply distinct from a **Note** (trip-wide planning text, part of the Trip's content) and from a **Flag** (a problem the app detects in a *plan* and asks the Traveller to fix). A Flag is about the holiday; a Feedback note is about the software.
_Avoid_: Note (that's trip content), Flag (that's a plan problem), bug, issue, ticket, report, comment

**Site** (of a Feedback note):
Which deployment of TEEPEE a **Feedback note** was written on — `main` (the live site), `beta` (the preview of work not yet on the live site), another preview branch by name, or `local`. Recorded by the server, never chosen by the author, so a remark about something only beta has can't be mistaken for one about the live site. Notes written before sites were recorded count as `main`.
_Avoid_: environment, version, build
...
**Release note**:
One line telling a **Traveller** about a change to TEEPEE they would actually notice — "Attachments now open in a new tab". Like a **Feedback note** it is about the *software*, not the holiday: it belongs to no **Trip** and no **Plan**, and nothing in any planning view reads it. It is the outward half of the conversation a Feedback note starts — a Traveller tells the operator what is wrong, a Release note tells the Travellers what was done — so a Release note that closes a Feedback note says whose it was, because the person who bothered to write one should see it land. Written by the operator and shipped alongside the code it describes, which means a Release note becomes true on *deploy* rather than on commit: several written across a week all arrive together, as one release. Most of what TEEPEE ships earns no Release note at all — only a change a Traveller would notice does, and that, rather than any cap on **What's new**, is what keeps the news short. Distinct from an **Activity** (a thing a *Traveller* did to a *Trip*, which is that Trip's own shared history) and from a **Digest** (TEEPEE's outward push about the *holiday*).
_Avoid_: Note (that's trip content), update (as a noun — **Activity** avoids it for the same reason), changelog entry, announcement, patch note, version

**What's new**:
The surface **Release note**s are read on, in two parts. A compact, dismissible card greets a Traveller on *arrival* — the trips list and a Trip's **Home**, never inside a Day view or the plan editor, which are places you are using the app rather than returning to it — carrying at most three unread Release notes and a link to the rest, so however large a release is the interruption stays a fixed small size. Behind the avatar menu sits the full dated list, which is always there to go back to and is the only part that is ever navigated to deliberately. Dismissing the card marks *every* Release note it covered as read, not only the three it showed, so news a Traveller chose not to read never returns to ask again. What counts as unread is a fact about the *person* rather than the browser (see **Account**), so it follows a Traveller to every device they sign in on; and a Traveller who has dismissed nothing — a brand-new one above all — starts caught up rather than meeting the entire history at once, because to them none of it is new, it is simply how the app is.
_Avoid_: Changelog, release notes (as the name of the surface — that's the plural of the entry), news feed, announcements, banner, modal
