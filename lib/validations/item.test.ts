...
// ---------------------------------------------------------------------------
// itemSchema — paidAt calendar validity and amount cap (AB-04)
// ---------------------------------------------------------------------------

describe("itemSchema paidAt calendar validity and amount cap", () => {
  ...
  it("rejects an amount above the shared minor-unit cap", () => {
    const result = itemSchema.safeParse({ ...VALID_UNSCHEDULED, costMinor: 2_147_483_648 });
    expect(result.success).toBe(false);
  });

  it("accepts an amount at the shared minor-unit cap", () => {
    const result = itemSchema.safeParse({ ...VALID_UNSCHEDULED, costMinor: 2_147_483_647 });
    expect(result.success).toBe(true);
  });
});
