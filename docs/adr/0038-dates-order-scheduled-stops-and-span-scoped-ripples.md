...
2. **Dragging a scheduled Stop is a span-scoped date edit that preserves gaps.** Only the Stops between the old and new positions (inclusive) are re-dated, flowing from the affected span's original start; each unmoved Stop keeps its nights *and its lead-in free days*; everything outside the span keeps its exact dates. A drag never changes the trip's overall length as a side effect. Dragging a Chapter moves its block with the same semantics.
