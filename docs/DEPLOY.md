...
2. Add **Environment Variables** (Production):
   ...
   | `APP_URL` | `https://<your-vercel-domain>` (no trailing slash — same value as the GitHub Actions variable in §5). Resolves absolute `og:image` URLs (`lib/site-url.ts`); without it those URLs fall back to `http://localhost:3000` at build time. |
